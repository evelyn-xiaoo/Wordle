...
(define (string-in-list? s los)
  (ormap (lambda (string) (string=? s string)) los))
...
(define (read-dictionary f)
  (if (file-exists? f)
      (map string-upcase (read-lines f))
      '()))
...
(define (score guess answer)
  (local [; status : lString lString -> Pair
          ; makes a pair which also shows the letter status
          (define (status l1 l2)
            (cond [(string=? l1 l2) (make-pair l1 LS-RIGHT)]
                  [(string-in-list? l1 (explode answer)) (make-pair l1 LS-MISPLACED)]
                  [else (make-pair l1 LS-WRONG)]))]
    (mymap2 (explode guess) (explode answer) status)))
...
(define (partial-guess->full-guess guess needed-length)
  (if (< (length (explode guess)) needed-length)
      (implode (append (explode guess) (build-list (- needed-length (string-length guess))
                                                   (lambda (x) " "))))
      guess))
...
(define (key-wordle ws ke los n)
  (cond [(string=? "\b" ke) (backspace ws)]
        [(string=? "\r" ke) (return ws los n)]
        [(and (< (string-length (wordle-current ws)) n)
              (string-contains? ke "abcdefghijklmnopqrstuvwxyzABCDEEFGHIJKLMNOPQRSTUVWXYZ"))
         (add-letter ws ke)]
        [else ws]))
...
(define (backspace ws)
  (if (= (string-length (wordle-current ws)) 0)
      ...
      (make-wordle (wordle-past ws)
                   (substring (wordle-current ws)
                              0
                              (- (string-length (wordle-current ws)) 1)))))

; returns the same if input isnt valid 
(define (return ws los n)
  (if (= (string-length (wordle-current ws)) n)
      (in-list? ws los)
      ws))

; in-list? : WS [List-of String] -> WS
(define (in-list? ws los)
  (cond [(empty? los) ws]
        [(cons? los) (if (string=? (wordle-current ws) (first los))
                         (add-to-past ws)
                         (in-list? ws (rest los)))]))

; add-letter : WS KeyEvent -> WS
(define (add-letter ws ke)
  (make-wordle (wordle-past ws) (string-append (wordle-current ws) (string-upcase ke))))
                                      
; add-to-past : WS -> WS
(define (add-to-past ws)
...
(define (end-wordle? ws correct n)
  (cond [(= (length (wordle-past ws)) n) #true]
        [(ormap (lambda (guess) (string=? correct guess)) (wordle-past ws)) #true]
        [else #false]))
...
(check-expect (draw-end-wordle (make-wordle (list "hat" "bad") "") "bad" 4)
              (overlay
               (above/align "left"
                            (text "YOU WON! :)"
                                  (* 8 3)
                                  "white")
                            (text (string-append "guesses: 2")
                                  (* 4 3)
                                  "white"))
               (rectangle (* 70 3)
                          (* 70 4)
                          "solid"
                          "black")))
(check-expect (draw-end-wordle WS-START "berth" 6)
              (overlay
               (above/align "left"
                            (text "YOU LOST :("
                                  40
                                  "white")
                            (text "the correct word was berth"
                                  20
                                  "white"))
               (rectangle 350
                          420
...
(define (draw-end-wordle ws correct n)
  (if (ormap (lambda (guess) (string=? correct guess)) (wordle-past ws))
      (winner ws correct n)
      (loser correct n)))

; winner : WS String Nat -> Image
; produces the winning screen with the amount of guesses
(check-expect (winner (make-wordle (list "hat" "bad") "") "bad" 4)
              (overlay
               (above/align "left"
                            (text "YOU WON! :)"
                                  (* 8 3)
...
(check-expect (winner (make-wordle (list "berth") "") "berth" 6)
              (overlay
               (above/align "left"
                            (text "YOU WON! :)"
                                  (* 8 5)
                                  "white")
                            (text (string-append "guesses: 1")
                                  (* 4 5)
                                  "white"))
               (rectangle (* 70 5)
                          (* 70 6)
...
(define (winner ws correct n)
  (overlay
   (above/align "left"
                (text "YOU WON! :)"
                      (* 8 (string-length correct))
                      "white")
                (text (string-append "guesses: "
                                     (number->string (length (wordle-past ws))))
                      (* 4 (string-length correct))
                      "white"))
   (rectangle (* 70 (string-length correct))
              (* 70 n)
              "solid"
              "black")))
 
; loser : WS String Nat -> Image
; produces the losing screen and reveals the correct word
(check-expect (loser "bad" 4)
              (overlay
               (above/align "left"
                            (text "YOU LOST :("
                                  24
                                  "white")
                            (text "the correct word was bad"
                                  12
                                  "white"))
               (rectangle 210
                          280
...
(define (loser correct n)
  (overlay
   (above/align "left"
                (text "YOU LOST :("
                      (* 8 (string-length correct))
                      "white")
                (text (string-append "the correct word was "
                                     correct)
                      (* 4 (string-length correct))
                      "white"))
   (rectangle (* 70 (string-length correct))
...
(define (draw-wordle ws correct n)
  (local [(define past-guess
            (lop->image ws correct n))
          (define current-guess
            (partial-guess->image (wordle-current ws) (string-length correct)))
          (define future-guess
            (build-list (- n (add1 (length (wordle-past ws)))) 
                        (lambda (nat) (partial-guess->image "" (string-length correct)))))]
    (stack/v (append past-guess
                     (cons current-guess future-guess)))))

; lop->image : WS String Nat -> [List-of Image]
; make an image for each [List-of Pair]
(define (lop->image ws correct n)
  (map row->image
       (make-row ws correct n)))

; make-row : WS String Nat -> [List-of [List-of Pair]]
; makes a pair for each word guessed
(define (make-row ws correct n)
  (map (lambda (lolop) (map (lambda (lsp) (make-pair lsp lsp->image)) lolop))
       (guess->score ws correct n)))

; guess->score : WS String Nat -> [List-of [List-of Pair]]
; scores each word guessed
(define (guess->score ws correct n)
  (map (lambda (guess) (score guess correct))
       (wordle-past ws)))
